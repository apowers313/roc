...
# Object UUIDs are 63-bit ints generated by ROC's ID source. They exceed
# JS Number.MAX_SAFE_INTEGER (2^53 - 1), so the wire format must serialize
# them as strings -- a JS client parsing them as numbers would silently
# truncate the trailing digits and corrupt every UUID-keyed lookup. We
# convert at to_cytoscape() time so all graph endpoints get the fix.
_UUID_FIELDS: frozenset[str] = frozenset({"uuid", "object_uuid"})

# Stable, human-readable name for an Object derived from its UUID. Used by
# the dashboard's detail panels so users can recognise objects at a glance
# instead of squinting at 19-digit ints. Format: ``{adj}-{first}-{last}``,
# e.g. ``rancorous-fey-devy``. Build the hasher once because the
# constructor walks the template and is non-trivial.
_OBJECT_NAME_TEMPLATE = "{{adj|lower}}-{{firstname|lower}}-{{lastname|lower}}"
_object_name_hasher = FlexiHumanHash(_OBJECT_NAME_TEMPLATE)


def object_human_name(uuid: int | str) -> str:
    """Deterministic human-readable name for an object UUID.

    Accepts the UUID as either int (graph internal) or str (wire format).
    Two calls with the same UUID return the same name.
    """
    if isinstance(uuid, str):
        uuid = int(uuid)
    return str(_object_name_hasher.hash(uuid))
...
class GraphService:
    ...
    def to_cytoscape(G: nx.DiGraph, root_id: int | None = None) -> dict[str, Any]:
        ...
        Object UUIDs (``uuid``, ``object_uuid``) are 63-bit ints that
        exceed JS Number's safe range, so they're emitted as strings.
        Object nodes also get a derived ``human_name`` field for display.
        ...
        for node_id, data in G.nodes(data=True):
            ...
            node_data.update(GraphService._normalize_node_data(data))
    ...
    @staticmethod
    def _normalize_node_data(data: dict[str, Any]) -> dict[str, Any]:
        """Apply wire-format conversions to a node's attribute dict.

        - ``uuid`` / ``object_uuid`` ints become strings (precision-safe).
        - Object nodes get a ``human_name`` field derived from the uuid.
        """
        out: dict[str, Any] = {}
        for k, v in data.items():
            if k in _UUID_FIELDS and isinstance(v, int) and not isinstance(v, bool):
                out[k] = str(v)
            else:
                out[k] = v
        # Add a derived display name for any node carrying a uuid -- both
        # Object and ObjectInstance benefit (instance shows the parent's
        # name; the dashboard already groups them by uuid).
        raw_uuid = data.get("uuid") if "uuid" in data else data.get("object_uuid")
        if isinstance(raw_uuid, int) and not isinstance(raw_uuid, bool):
            try:
                out["human_name"] = object_human_name(raw_uuid)
            except Exception:
                # Hash failures must never break a graph fetch -- the
                # dashboard can fall back to the raw uuid string.
                pass
        return out
